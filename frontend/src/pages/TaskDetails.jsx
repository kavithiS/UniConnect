...
const TaskDetails = () => {
  ...
  const isSubtaskDone = (subtask) =>
    subtask.status === 'done' || subtask.completed === true;
  ...
  const canMarkAsDone = () => areAllSubtasksDone();
  ...
  return (
    <div className="animate-fadeIn max-w-4xl mx-auto w-full">
      ...
      {/* Back */}
      <Link to="/dashboard/tasks" className="btn btn-secondary inline-flex mb-6 text-sm py-2 px-4">
      ...
      <div className="glass-panel p-8">
        ...
        {/* Subtasks */}
        <div className="p-6 rounded-xl mb-8 bg-gray-100 text-gray-900 dark:bg-slate-900/40 dark:text-slate-100">
          <div className="flex items-center justify-between mb-4">
            ...
            {task.subtasks && task.subtasks.length > 0 && (
              <span className="text-sm text-gray-600 dark:text-slate-400">
                {task.subtasks.filter(isSubtaskDone).length} / {task.subtasks.length} completed
              </span>
            )}
          </div>

          {!task.subtasks || task.subtasks.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-slate-400">No subtasks added for this task.</p>
          ) : (
            <>
                ...
                {task.subtasks.map((subtask, index) => (
                  <li key={index} className="flex items-center gap-3 p-3 bg-white/50 rounded-lg dark:bg-slate-800/50">
                    <input
                      ...
                      onChange={() => updateSubtaskStatus(index, isSubtaskDone(subtask) ? 'todo' : 'done')}
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                    <span className={`flex-1 ${isSubtaskDone(subtask) ? 'line-through text-gray-500' : ''}`}>
                      {subtask.title}
                    </span>
                    <span className={`text-xs px-2 py-1 rounded ${
                      isSubtaskDone(subtask) ? 'bg-green-100 text-green-800' :
                      subtask.status === 'inprogress' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-slate-300'
                    }`}>
                      {isSubtaskDone(subtask) ? 'Done' : subtask.status === 'inprogress' ? 'In Progress' : 'To Do'}
                    </span>
                  </li>
                ))}
              </ul>
