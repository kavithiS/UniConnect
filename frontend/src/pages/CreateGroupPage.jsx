  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (trimmedTitle.length < MIN_TITLE_LENGTH) {
        setError(`Group title must be at least ${MIN_TITLE_LENGTH} characters long.`);
        setLoading(false);
        return;
      }
      ...
      const formattedGroupId = rawGroupId?.startsWith('IT100-')
        ...
        : rawGroupId
          ...
          : 'IT100-UNKNOWN';
      setCreatedGroupId(formattedGroupId);
      setSuccess(`✅ Group created successfully! Your Group ID is: ${formattedGroupId}`);
  ...
  return (
    <div className={`min-h-screen p-6 ${isDarkMode ? 'bg-gradient-to-br from-slate-950 to-slate-900' : 'bg-gradient-to-br from-slate-50 to-slate-100'}`}>
      ...
      <div className="mb-8">
        ...
        <h1 className={`text-4xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Create New Group</h1>
        <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Set up a new collaboration group</p>
      </div>

      {/* Form Container */}
      <div className="max-w-2xl mx-auto">
        <div className={`border rounded-lg p-8 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
          {error && (
          ...
          {success && (
            <div className={`mb-6 p-4 border rounded-lg ${isDarkMode ? 'bg-green-500/20 border-green-500 text-green-300' : 'bg-green-100 border-green-300 text-green-700'}`}>
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                required
              />
              <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Minimum {MIN_TITLE_LENGTH} characters.</p>
            </div>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                required
              />
              <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Minimum {MIN_DESCRIPTION_LENGTH} characters.</p>
            </div>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className={`w-full rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 transition border ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-slate-100 border-slate-300 text-slate-900'}`}
              />
              <p className={`text-sm mt-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Separate skills with commas</p>
            </div>
            ...
            <button
              ...
              className={`w-full py-3 px-6 rounded-lg font-semibold text-white transition ${
                loading
                  ? isDarkMode ? 'bg-slate-600 cursor-not-allowed' : 'bg-slate-400 cursor-not-allowed'
