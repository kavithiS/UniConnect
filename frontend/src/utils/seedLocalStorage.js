...
async function seedDemoUser() {
  try {
    ...
    const userExists = async (apiBase, id) => {
      ...
      try {
        ...
        const inStudents =
          ...
          students.some((s) => (s?._id || s?.id) === id);
        const inUsers =
          Array.isArray(users) && users.some((u) => (u?._id || u?.id) === id);
    ...
    if (existingUserId && !isObjectId(existingUserId)) {
      ...
      console.info(
        "seedLocalStorage: removed invalid userId from localStorage",
      );
    }
    ...
    // Only seed profile state when an auth session already exists.
    // This avoids repopulating userId after logout and keeps re-login clean.
    if (!localStorage.getItem("authToken")) {
      // eslint-disable-next-line no-console
      console.info("seedLocalStorage: no auth token, skipping user seeding");
      return;
    }
    ...
    if (currentId) {
      ...
      if (!exists) {
        ...
        // eslint-disable-next-line no-console
        console.info(
          "seedLocalStorage: removed stale userId from localStorage",
        );
      } else {
        // eslint-disable-next-line no-console
      ...
      if (!localStorage.getItem("userId")) {
        ...
        if (res.ok) {
          ...
          if (id && isObjectId(id)) {
            ...
            // eslint-disable-next-line no-console
            console.info(
              "seedLocalStorage: seeded userId from /projects/seed",
              id,
            );
          }
        } else {
          // eslint-disable-next-line no-console
          console.warn(
            "seedLocalStorage: failed to seed project/users",
            res.status,
          );
        }
      }
      ...
      if (!localStorage.getItem("projectId")) {
        ...
        if (res.ok) {
          ...
          const pid =
            ...
            (project.seedData &&
              project.seedData.groupIds &&
              project.seedData.groupIds[0] &&
              project.seedData.groupIds[0]._id);
          if (pid) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            "seedLocalStorage: failed to seed/fetch project",
            res.status,
          );
        }
      }
